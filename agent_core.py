...
def new_session(user_message: str) -> dict:
    ...
    system_prompt = (
        ...
        f"placeholder text. NEVER invent details that are not present in a "
        f"tool result — this includes URLs, links, IDs, or any other data. "
        f"If a tool result doesn't include a link, do not create one. "
        f"You may call multiple tools, one after another, "
